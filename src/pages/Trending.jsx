...
import { displayCards } from "../components/MediaCard";
import {
    getMediaData,
    getTrendingWeek,
    getTrendingDay,
    ...
    useEffect(() => {
        async function loadMedia() {
            try {
                ...
                setAllTrending(getMediaData(response.results));
                setHasLoaded(true);
            } catch (error) {
                console.error(error);
            }
    ...
    return (
        <div className="bg-background min-h-screen text-text font-bold">
            <div className="mx-auto max-w-7xl py-10 px-5 sm:px-8 lg:px-10">
                <div className="flex items-center">
                    ...
                    <div className="flex items-center w-fit h-fit ml-auto text-xs md:text-sm border border-surface rounded-2xl">
                        <button
                            ...
                            className={`${!toggle && "bg-surface text-text-secondary"} px-4 md:px-6 py-2 rounded-2xl transition duration-300`}
                        >
                            Day
                        </button>
                        <button
                ...
                {hasLoaded ? (
                    <div className="grid grid-cols-[repeat(auto-fit,minmax(150px,1fr))] md:grid-cols-[repeat(auto-fit,minmax(185px,1fr))]  gap-5 md:gap-8 py-10">
